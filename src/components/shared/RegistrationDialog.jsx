...
function RegistrationDialog({ children, courseId = null, courseTitle, triggerProps }) {
    ...
    async function onSubmit(values) {
        ...
        if (result.success) {
            toast.success("Gửi thông tin thành công!", {
                description: "Cảm ơn bạn đã quan tâm. A&U sẽ liên hệ với bạn sớm nhất.",
            });
            form.reset();
            setOpen(false);
        } else {
            ...
            <DialogContent className="sm:max-w-2xl bg-white p-8">
                <DialogHeader>
                    <DialogTitle className="text-2xl font-bold text-gray-900">
                        {courseTitle ? `Đăng ký khóa học: ${courseTitle}` : 'Đăng ký tư vấn'}
                    </DialogTitle>
                    <DialogDescription>
